...
enum Card {
    N2 = 2, N3, N4, N5, N6, N7, N8, N9, T, J, Q, K, A
}
...
impl Hand {
    fn strength(&self) -> Strength {
        ...
        match m.len() {
            ...
            2 => {
                let mut counts = m.values();
                match (counts.next().unwrap(), counts.next().unwrap()) {
                    (4, 1) | (1, 4) => Strength::FourOfAKind(&self.cards),
                    (3, 2) | (2, 3) => Strength::FullHouse(&self.cards),
                    _ => unreachable!(),
                }
            },
            3 => {
                let mut counts = m.values();
                match (counts.next().unwrap(), counts.next().unwrap(), counts.next().unwrap()) {
                    (3, 1, 1) | (1, 3, 1) | (1, 1, 3) => Strength::ThreeOfAKind(&self.cards),
                    (2, 2, 1) | (2, 1, 2) | (1, 2, 2) => Strength::TwoPair(&self.cards),
                    _ => unreachable!(),
                }
            },
